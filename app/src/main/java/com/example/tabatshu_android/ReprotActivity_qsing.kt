...
class ReprotActivity_qsing : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        enableEdgeToEdge()
        setContentView(R.layout.activity_reprot_qsing)
        ...
        reportTypeEditText = findViewById(R.id.reporttypebtn)
        ...
        // 신고 유형 EditText 설정 (큐싱 의심)
        reportTypeEditText.apply {
            setText("큐싱 의심")
            isFocusable = false
    ...
    private fun handleReportButtonClick() {
        ...
        val reportType = "큐싱의심" // Hardcoded value
        val imageSelect = imageSelectEditText.text.toString().trim()
        val message = messageEditText.text.toString().trim()

        // 입력 검증
        if (name.isEmpty() || email.isEmpty() || imageSelect.isEmpty() || message.isEmpty()) {
        ...
        call.enqueue(object : Callback<ResponseBody> {
            override fun onResponse(call: Call<ResponseBody>, response: Response<ResponseBody>) {
                if (response.isSuccessful) {
                    ...
                    val intent = Intent(this@ReprotActivity_qsing, HomeActivity::class.java)
